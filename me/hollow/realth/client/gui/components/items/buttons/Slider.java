...
public class Slider extends Button
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, (((Number)this.setting.getValue()).floatValue() <= this.min.floatValue()) ? this.x : (this.x + (this.width + 7.4f) * this.partialMultiplier()), this.y + this.height - 0.5f, this.getColor(true));
        JordoHack.fontManager.drawString(this.getName().toLowerCase() + ": " + ChatFormatting.GRAY + ((this.setting.getValue() instanceof Float) ? ((Number)this.setting.getValue()) : Double.valueOf(((Number)this.setting.getValue()).doubleValue())), this.x + 2.3f, this.y - 1.7f - TrollGui.getClickGui().getTextOffset(), -1);
    }
    ...
    private void setSettingFromX(final int mouseX) {
        ...
        if (this.setting.getValue() instanceof Double) {
            ...
            this.setting.setValue((Object)(Math.round(10.0 * result) / 10.0));
        }
        else if (this.setting.getValue() instanceof Float) {
            final float result2 = (float)this.setting.getMin() + this.difference * percent;
            this.setting.setValue((Object)(Math.round(10.0f * result2) / 10.0f));
        }
        else if (this.setting.getValue() instanceof Integer) {
            this.setting.setValue((Object)((int)this.setting.getMin() + (int)(this.difference * percent)));
        }
    }
